...
.text
	...
	readfile:
		...
		li   $a2, 4000      # quantidade de bytes para ler
		syscall             # lê o arquivo
			
		la $t0, buffer		# carrega o buffer
		while:
			
			lw $t6, pc			# carrega o pc
			lw $t2, buffer($t6) # pega a instrução na posição pc
			beqz $t2, exit		# se for zero significa que acabou
			move $a1, $t2		# posiciona a instrução em decimal
			jal print_binary	# transforma em binario
			
			addi $t6, $t6, 4	# incrementa pc em 4
			sw $t6, pc			# salva pc
